...
@section('content')
    <section class="w-9/12 min-h-screen flex flex-col items-center py-24">
        ...
        <form action="{{ route('user.books.search') }}" method="POST" class="w-full my-8">
            @csrf
                ...
                @foreach ($books as $book)
                    <div
                        ...
                        <div class="flex gap-4 mt-2">
                            ...
                            @if ($book->customers->where('id', session('customer'))->first())
                                <p
                                    class="mb-4 text-sm px-6 pb-2.5 pt-2.5 bg-yellow-800 text-white font-bold rounded tracking-wider w-fit uppercase leading-normal text-white shadow-warning-3 transition duration-150 ease-in-out cursor-not-allowed dark:shadow-black/30 dark:hover:shadow-dark-strong dark:focus:shadow-dark-strong dark:active:shadow-dark-strong">
                                    Reserved
                                </p>
                            @else
                                <form onsubmit="return confirm('Apakah Anda Yakin ?');"
                                    action="{{ route('user.books.reserve', ['id' => $book->id]) }}" method="POST">
                                    @csrf
                                    @method('POST')
                                    <button type="submit"
